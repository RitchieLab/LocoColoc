...
if (is.numeric(chr) == FALSE) {
  if (grep("HSCHR", chr) == 1)
    chr <- as.numeric(substr(chr, nchar("HSCHR") + 1, nchar("HSCHR") + 1))
  else stop("Chromosome as integer (1 to 23) not specified")
}
...
if (gwas.response.type%in%c("cc", "quant") == FALSE)
  stop("Response type should either be 'cc' or 'quant'")
...
if (length(eqtl.sample.size.file) == 0)
  stop("eQTL sample size file for all tissues in GTEX v8 not specified")
...
if (length(grep(paste0(
      c("variant_id", "panel_variant_id", "chromosome", "position", "effect_allele",
        "non_effect_allele", "frequency", "zscore", "effect_size", "standard_error",
        "pvalue", "sample_size", "n_cases"), collapse="|"), colnames(gwas), ignore.case = TRUE)) < 13)
  stop("Required columns are not present in the requested format in the GWAS dataset")

colnames(gwas) = c("SNP", "panel_variant_id", "CHR", "BP", "A1", "A2", "A1FREQ", "ZSCORE", "BETA", "SE", "P", "N", "NCASES")

if (gwas.response.type == "cc") 
  gwas[,"BETA"] <- gwas[, "ZSCORE"]*gwas[, "SE"]

gwas[, "CHR"] = substr(gwas[, "CHR"],4, nchar(gwas[, "CHR"]))
gwas <- gwas[which(gwas[, "CHR"] == chr),]

if (nrow(gwas) == 0)
  stop("No SNPs in corresponding chromosome; check GWAS file and chromosome specification")
...
if (nrow(gwas.fin) == 0)
  stop("No sig GWAS hits for given trait in this chromosome")
...
if (run.cojo) {
  ...
  if (length(reference.file.bim) == 0)
    stop("Reference filename is not in .chromosome.bim/bed/fam format")
  else if (length(reference.file.bim) > 1)
    stop("More than one reference filename")
...
if (nrow(genes.fin) == 0)
  stop("Genes(s) of interest not in file")
...
if (length(tmp)==0) {
  message("No GWAS 'lead' SNPs within specified boundaries of this gene")
  quit()
}
...
if (length(unique(eqtl.fin[,"CHR"])) == 0)
  stop("eQTL allpairs dataset does not correspond to the chromosome specified")
...
if (length(common_lead_snps) == 0)
  message(paste0("No GWAS 'lead' SNPs have p-value < specified threshold of ", eqtl.p.threshold, " in the eQTL dataset"))
...
getWindow.fn <- function(data, baseRow, size=window.size, jump) {
  ...
  while (!success) {
    ...
    while (success2.3) {
      ...
      success2.3 <- (tmp2 < extraDistance) & topRow > jump + 1
...
for (j in 1:length(lead_snps)) { # Loop starts for lead snps
  ...
  for (i in 1:ncol(coloc_probs)) {
    coloc_probs[, i] <- as.character(unlist(coloc_probs[, i]))
  }

  base <- which(paste0(gwas[, "CHR"], ":", gwas[, "BP"]) == varid)[1]

  ## Get boundary rows for GWAS dataset; these will yield all the GWAS SNPs within chosen window size on either side of lead SNP
  boundaryRows <- data.frame(getWindow.fn(baseRow=base, data=gwas, size=lead.snp.window.size, jump=100))

  if(boundaryRows[[1]] < 0)
    boundaryRows[[1]] = 1
  x_gwas <- gwas[boundaryRows[[1]]:boundaryRows[[2]], ]

  if (length(which(x_gwas[, "A1FREQ"]%in%c(0, 1, NA))) > 0)
    x_gwas <- x_gwas[-which(x_gwas[, "A1FREQ"]%in%c(0, 1, NA)), ]
  ...
  if (nrow(eqtl.fin2) == 0) {
    ...
    message("No variants within specified boundaries of lead SNP in the eQTL dataset")
    next
  }

  x_eqtl <- cbind(eqtl.fin[which(eqtl.fin[, "varID"]%in%x_gwas[, "varID"]), ], "N"=N.eqtl.tissue)

  if (length(which(x_eqtl[,"maf"]==0)) > 0)
    x_eqtl <- x_eqtl[-which(x_eqtl[, "maf"] == 0), ]

  if (gwas.response.type == "quant")
    gwas.coloc <- x_gwas[, c("CHR", "varID", "BP", "A1", "A1FREQ", "BETA", "SE", "P", "N")]
  ...
  if (gwas.response.type == "quant")
    colnames(gwas.coloc) = colnames(eqtl.coloc)

  if (gwas.response.type=="cc")
    colnames(gwas.coloc) = c("Chr", "SNP", "bp", "refA", "freq", "b", "se", "p", "n", "s")
  ...
  if (nrow(eqtl.coloc) > 0) {
    if (gwas.response.type == "quant") {
      ...
      if (length(which(is.na(coloc_data[, "eqtl.p"])|is.na(coloc_data[, "gwas.p"]))) > 0)
        coloc_data <- coloc_data[-which(is.na(coloc_data[, "eqtl.p"])|is.na(coloc_data[, "gwas.p"])), ]
  ...
  if (coloc$value[[1]][[5]] < 0.5 & coloc$value[[1]][[6]] > 0.75)
  ...
  if (gwas.response.type=="cc")
    coloc_probs[,"GWAS.NCASES"] <-  x_gwas[which(x_gwas[,"varID"]==varid),"NCASES"][1]
  ...
  if(run.cojo){
    if (eqtl.coloc[which(eqtl.coloc[,"SNP"] == eqtl_topsnp),"p"] <= eqtl.p.threshold) {
      use.conditional.probs <- TRUE
    ...
    if(use.conditional.probs) {
      ...
      if (length(which(is.na(ref.common.varid))) == nrow(x_eqtl)) {
        next 
        message("No SNPs matching between reference and eQTL datasets; are reference and eQTL data on the same genome build?")
      }
      ...
      if (length(which(is.na(x_gwas_gcta[,"SNP"]))) > 0)
        x_gwas_gcta <- x_gwas_gcta[-which(is.na(x_gwas_gcta[,"SNP"])),]
      ...
      if (length(which(duplicated(as.character(unlist(x_eqtl_gcta[,"SNP"]))))) > 0)
        x_eqtl_gcta[which(duplicated(x_eqtl_gcta[,"SNP"])&(nchar(x_eqtl_gcta[,"A1"]) > 0|nchar(x_eqtl_gcta[,"A2"]) > 0)),] <- NA

      if (length(which(is.na(x_eqtl_gcta[,"SNP"]))) > 0)
        x_eqtl_gcta <- x_eqtl_gcta[-which(is.na(x_eqtl_gcta[,"SNP"])),]
      ...
      if (nrow(coloc_gcta) == 0) {
        message("tissue=",tissue," / j=",j," / ",trait," / ",gwas.data.name,"  / num.lead.snps=",length(lead_snps))
      ...
      if (coloc$value[[1]][[5]] < 0.5 & coloc$value[[1]][[6]] > 0.5) {
         if (nrow(coloc_topsnps) > 0) {
          eqtl_topsnp2 <- coloc_topsnps[which(coloc_topsnps[,"eqtl.pval_nominal"] == min(coloc_topsnps[,"eqtl.pval_nominal"])),"gwas.SNP"][1]
         } else {
          eqtl_topsnp2 <- as.character(unlist(x_eqtl_gcta[which(x_eqtl_gcta[,"pval_nominal"] == min(x_eqtl_gcta[,"pval_nominal"],na.rm=T)),"SNP"][1]))
         }
      ...
      #7.1 Run GCTA-COJO to perform association analysis of all the included SNPs conditional on the given top SNP in the GWAS dataset. Results are saved in the .cma.cojo file
         ...
         if (error.1)
          tmp.1pC <- NA else tmp.1pC <- tmp.1$value[,"pC"]
      ...
      if(error.2)
        tmp.2pC <- NA else tmp.2pC <- tmp.2$value[,"pC"]
      ...
      if(error.2) {
        badsnps = read.table(paste0("intermediate/",tissue,"_chr_",chr,"_pos_",pos,"_",gene_name,"_conditionalP.freq.badsnps"),header=T)
        eqtl_topsnps3 = eqtl_topsnps2[-which(eqtl_topsnps2%in%badsnps[,"SNP"])]
        if (length(eqtl_topsnps3)==0) {
          write.table(coloc_probs, file=paste0("chr",chr,"_",gwas.data.name,"_",trait,"_",tissue,"_",gene_name,"_colocProbs.txt"),row.names=F,col.names=F,quote=F,sep="\t",append=T)
      ...
      #7.3 Prepare eQTL and GWAS datasets for colocalization analysis between GWAS and eQTL datasets using conditional probabilites from GCTA-COJO 
        if (gwas.response.type == "quant") {
          ...
          if (length(which(is.na(coloc_data2[,"eqtl.p"])|is.na(coloc_data2[,"gwas.p"]))) > 0)
            coloc_data2 <- coloc_data2[-which(is.na(coloc_data2[,"eqtl.p"])|is.na(coloc_data2[,"gwas.p"])),]
        ...
        if ((coloc2$value[[6]] < 0.2) | (coloc2$value[[5]] > 0.5))
          message("\nNo support for secondary signals in this region.\n")
        if ((coloc2$value[[5]] < 0.5) & (coloc2$value[[6]] > 0.25) & (coloc2$value[[6]] <= 0.5)) {
          message("\nModerate support for secondary signals in this region!\n")
        }
        if (coloc2$value[[5]] < 0.5 & coloc2$value[[6]] > 0.5 & coloc2$value[[6]] <= 0.75) {
          message("\nGood support for secondary signals in this region!\n")
        }
        if (coloc2$value[[5]] < 0.5 & coloc2$value[[6]] > 0.75) {
